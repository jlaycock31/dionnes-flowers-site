...
// ── 1. New animation <style> block for <head> ─────────────────────────────────
...
// Insert the animation style block before </head>
src = src.replace(
  '<link rel="stylesheet" href="css/shared.css" />\n</head>',
  '<link rel="stylesheet" href="css/shared.css" />\n' + ANIM_STYLE + '\n</head>'
);

// ── 2. Remove the old animation <style> block (between </style> and <div class="hero">) ──
src = src.replace(
  /\n<style>\n@media\(prefers-reduced-motion:no-preference\)\{[\s\S]*?\}\n<\/style>\n(?=<div class="hero">)/,
  '\n'
);

// ── 3. Rename hero container class hero-img → hero-illustration ───────────────
src = src.replace(
...
// ── 4. Replace the full SVG content with re-grouped version + bees ────────────
...
const NEW_SVG = `<svg id="hero-svg" viewBox="0 0 340 380" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" style="position:absolute;top:0;left:0">
...
// Replace old SVG (from opening tag to </svg>)
src = src.replace(
  /<svg id="hero-svg"[\s\S]*?<\/svg>/,
  NEW_SVG
);

// ── 5. Mobile: ensure hero-illustration has no extra top gap ──────────────────
// The existing 1024px rule already covers .hero-illustration; add explicit rule
src = src.replace(
  '  .hero-text{padding-bottom:0.5rem!important}\n}',
  '  .hero-text{padding-bottom:0.5rem!important}\n  .hero-illustration{margin-top:0!important;padding-top:0!important}\n}'
);

// ── 6. Replace observer script (class name + selector) ────────────────────────
const OLD_OBSERVER = `<script>
(function(){
...
src = src.replace(OLD_OBSERVER, NEW_OBSERVER);

if (src === orig) {
  console.error('ERROR: no changes made — check string matches');
  process.exit(1);
}

fs.writeFileSync(fp, src, 'utf8');
console.log('Done — index.html updated with three-phase animation + bees.');
